...
export const RecommendContent: FC = () => {
  const recommendProducts = useRecoilValue(recommendProductsState);
    ...
    // Lọc chỉ các sản phẩm có categoryId: ["Services"]
    const filteredProducts = recommendProducts.filter((product) =>
    product.categoryId.includes("Services")
  );

  return (
    <Section title="Dịch vụ nổi bật" padding="title-only">
...
export const RecommendFallback: FC = () => {
  const recommendProducts = [...new Array(4)];

  return (
    <Section title="Nổi bật" padding="title-only">
      <Swiper slidesPerView={1.25} spaceBetween={16} className="px-4">
